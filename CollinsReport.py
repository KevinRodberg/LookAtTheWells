...
def makeChartsByAquifer(options, DBKeyList):
        ...
        #If options.rfgw build and prcess query for WQ data.
                if options.rfgw:
                        sqlWQ = "SELECT TO_DATE(b.daily_date, 'MM/DD/YYYY') daily_date, " +\
                            "           wq.SP_COND, wq.CHLORIDE, WQ.TDS FROM " +\
                            "          (SELECT * FROM " +\
                            ...
                            "                   IN (9 AS SP_COND, 32 AS CHLORIDE, 97 AS TDS))) wq " +\
                            "   RIGHT JOIN " +\
       ...
       #   Fetch WQ records if options set to rfgw
                        ...
                        WQtest = False
                        if len(wqrecs) > 0:
                            wqDF = pd.DataFrame(wqrecs,columns=['wqdate','spcond','chloride','tds'])
                            ...
                            yWQmax = float(max(yWQmaxSC, yWQmaxCL, yWQmaxTDS))
                            yWQmin = float(min(yWQminSC, yWQminCL, yWQminTDS))
