...
def getOptions():
    ...
    parser.add_argument("-aquifer",
                        dest="Aquifer",
                        choices=allAquifers,
                        help="Process 1 DBKeys for 1 Aquifer.")
...
def setSQLstring(qryName,keys='',rowData=None):
    if rowData:
        sdateDT = rowData.SDATE
        oneStartDate= rowData.START_DATE.strip(' ')
    if qryName == 'DBKeyDates':
        SQLstr = "SELECT d.dbkey,D.DATA_TYPE,D.STATION,D.FREQUENCY,D.AGENCY,d.START_DATE as sDATE," + \
              "      to_char(D.START_DATE,'MM/DD/YYYY') as START_DATE," + \
              "      to_char(D.END_DATE,'MM/DD/YYYY') as END_DATE FROM DM_KEYREF d " + \
              "WHERE d.DBKey in " + str(keys) + " AND d.frequency = ? ORDER BY D.STATION"
        ...
        SQLstr= "SELECT l.daily_date, r.VALUE, r.code FROM dm_daily_data l " +\
                ...
                "       ON l.DAILY_DATE = r.DAILY_DATE WHERE l.DBKEY = '" +rowData.DBKEY.strip(' ') +\
        ...
        SQLstr= "SELECT LISTAGG (code, ',') WITHIN GROUP (ORDER BY code) qualifiers " +\
                ...
                rowData.DBKEY.strip(' ')+ "'  AND C.DBKEY = b.dbkey " +\
        ...
        SQLstr= "SELECT l.daily_date, r.VALUE, r.code FROM dm_daily_data l " +\
                ...
                "    WHERE l.DBKEY = '" +rowData.DBKEY.strip(' ')+"' AND l.daily_date > " +\
...
def setDBKeylist(Aquifer):
    if Aquifer == 'Floridan':
        DBkeyList = ('88715','03576','PT638','OB341','W4254','OB342','OB343','OB344','P0061','P0062',
                     ...
                     'OG451','UP246','VN324','VM825','VM823','OG452','P6306','P6310','P6314','OB396','OB395',
